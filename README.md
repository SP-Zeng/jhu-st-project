...
**Tests**
  ...
  and Integration testing on the Backend in `\BACKEND\Home-Ecommerce-Backend\src\test\java\com\homecommerce\*` (Gradle)
  2. JUnit for Blackbox unit on the Backend Services (DTOs and Models are all an Entity classes to model the databases so it's not tested and Controllers are endpoints tested in the API testing) in `\BACKEND\Home-Ecommerce-Backend\src\test\java\blackbox_testing\services\` (Gradle)
  3. API Testing on the Backend in `\GUItesting\src\test\java`. Use Intellj, open the test file end with ".java" then press "ctrl + shift + F10" or press "Run" to go over tests. (Gradle)
  4. GUI Testing on the Backend in `\ApiTesting\src\test\java`. Use Intellj, open the test file end with ".java" then press "ctrl + shift + F10" or press "Run" to go over tests. (Gradle)
